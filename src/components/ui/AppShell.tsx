...
export function AppShell() {
  ...
  return (
    <div className="app-shell" onClick={() => { if (moreOpen) setMoreOpen(false); if (userMenuOpen) setUserMenuOpen(false); if (notifOpen) setNotifOpen(false); if (queryOpen) setQueryOpen(false) }}>
      <header className="app-header" onClick={e => e.stopPropagation()} style={{ paddingBottom: 12, marginBottom: 6 }}>
        <div style={{ width:'100%', padding:'14px 16px', borderRadius:24, background:'linear-gradient(180deg, var(--bg-card) 0%, var(--bg-soft) 100%)', border:'1px solid var(--border)', boxShadow:'var(--card-shadow)' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
            <div className="app-header-left">
              <img src="/apple-touch-icon.png?v=4" alt="Custodia" className="app-logo" style={{ width:44, height:44, borderRadius:14, boxShadow:'var(--card-shadow)', objectFit:'cover' }} />
              <div>
                <div style={{ fontSize:11, color:'var(--text-muted)', fontWeight:800, textTransform:'uppercase', letterSpacing:0.5, marginBottom:2 }}>Familia organizada</div>
                <div className="app-title" style={{ fontSize:22 }}>CustodiaApp</div>
                <div style={{ display:'flex', alignItems:'center', gap:8, position:'relative', marginTop:4 }}>
                  {child && <div className="app-subtitle" style={{ fontSize:13 }}>{child.name}</div>}
                  <div style={{ position:'relative' }}>
                    <button onClick={() => { setMoreOpen(false); setUserMenuOpen(false); setNotifOpen(false); setQueryOpen(v => !v) }} title="Consulta rápida" style={{ width:24, height:24, borderRadius:'50%', border:'1px solid var(--border-hover)', background:'var(--bg-card)', color:'var(--text-secondary)', fontSize:12, fontWeight:800, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>?</button>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, position: 'relative', flexWrap:'wrap', justifyContent:'flex-end' }}>
              <button onClick={() => { setMoreOpen(false); setUserMenuOpen(false); setNotifOpen(false); setQueryOpen(false); setSearchOpen(true) }} title="Buscar" style={{ height:40, minWidth:40, borderRadius:14, border:'1px solid var(--border-hover)', background:'var(--bg-card)', color:'var(--text-secondary)', cursor:'pointer', padding:'0 12px', display:'flex', alignItems:'center', justifyContent:'center', gap:6, boxShadow:'var(--card-shadow)' }}><span>🔎</span><span style={{ fontSize:11, fontWeight:800 }}>Buscar</span></button>
              {children.length > 1 && <select value={selectedChildId ?? ''} onChange={e => setSelectedChildId(e.target.value)} style={{ background:'var(--bg-card)', border:'1px solid var(--border-hover)', borderRadius:14, padding:'9px 12px', color:'var(--text-strong)', fontSize:12, outline:'none', boxShadow:'var(--card-shadow)' }}>{children.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>}
              <div style={{ position: 'relative' }}>
                <button className="notif-btn" onClick={() => { setMoreOpen(false); setUserMenuOpen(false); setQueryOpen(false); setNotifOpen(v => !v) }} style={{ width:40, height:40, borderRadius:14, background:'var(--bg-card)', border:'1px solid var(--border-hover)' }}>🔔{totalBadge > 0 ? <span className="notif-count">{totalBadge}</span> : null}</button>
              ...
              <div style={{ position: 'relative' }}>
                <button className="user-avatar" onClick={() => { setMoreOpen(false); setNotifOpen(false); setQueryOpen(false); setUserMenuOpen(v => !v) }} title="Usuario" style={{ width:40, height:40, borderRadius:14 }}>{user?.photoURL ? <img src={user.photoURL} alt="" /> : (user?.displayName ?? user?.email ?? 'U')[0].toUpperCase()}</button>
