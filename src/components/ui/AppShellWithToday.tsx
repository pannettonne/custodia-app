...
type Tab = 'today' | 'calendar' | 'requests' | 'notes' | 'events' | 'documents' | 'packing' | 'contacts' | 'medications' | 'stats' | 'settings'
...
const MORE_EVENTS_ICON = '/nav-icons/events.svg'
...
function notificationGroupLabel(type: AppNotification['type']) {
  ...
  if (type === 'event_reminder' || type === 'medication_reminder') return 'Recordatorios'
  return 'Otros'
}
...
function searchGroupLabel(type: SearchResultType) {
  ...
  if (type === 'document_folder') return 'Carpetas'
...
export function AppShellWithToday() {
  ...
  const allowedTabs: Tab[] = isParentForSelectedChild
    ? ['today', 'calendar', 'requests', 'notes', 'events', 'documents', 'packing', 'contacts', 'medications', 'stats', 'settings']
    : isCollaboratorForSelectedChild
      ? ['today', 'calendar', 'requests', 'contacts', 'medications', 'settings']
  ...
  useEffect(() => {
    ...
    const handler = (event: Event) => {
      ...
      setTab(allowedTabs.includes(detail.tab) ? detail.tab : 'calendar')
  ...
  const allSearchResults = useMemo<SearchResult[]>(() => {
    ...
    for (const c of children) {
      ...
      results.push({ id: `child-${c.id}`, type: 'child', title: c.name, subtitle: isParentForChild ? `${c.parents.length} progenitor(es)` : 'Colaborador', childId: c.id, targetTab: 'today' })
    ...
    if (isParentForSelectedChild) {
      ...
      for (const request of requests) {
        ...
        results.push({ id: `request-${request.id}`, type: 'request', title: request.reason || 'Propuesta de cambio', subtitle: `${childName} · ${dateLabel || ''} · ${request.status}`, childId: request.childId, date: request.date || request.startDate || undefined, endDate: request.endDate || undefined, targetTab: 'requests' })
      }
  ...
  const visibleMoreItems = isParentForSelectedChild ? [
    { id: 'events' as Tab, label: 'Eventos', icon: MORE_EVENTS_ICON },
  ...
  const activeMore = ['events', 'documents', 'packing', 'contacts', 'medications', 'stats', 'settings'].includes(tab)
  ...
  const openNotification = async (item: AppNotification) => {
    ...
    const nextTab = inferTargetTab(item)
    setTab(allowedTabs.includes(nextTab) ? nextTab : 'today')
    setNotifOpen(false)
  }
  ...
  const openSearchResult = (item: SearchResult) => {
    ...
    setTab(allowedTabs.includes(item.targetTab) ? item.targetTab : 'today')
    ...
    <div className="app-shell" onClick={() => { if (moreOpen) setMoreOpen(false); if (userMenuOpen) setUserMenuOpen(false); if (notifOpen) setNotifOpen(false); if (queryOpen) setQueryOpen(false) }}>
      ...
      <nav className="bottom-nav" onClick={e => e.stopPropagation()}>
        {mainTabs.map(({ id, label, icon, badge }) => {
          ...
          return <button key={id} className={`nav-btn ${isActive ? 'active' : ''}`} onClick={() => handleTabClick(id)}><img src={icon} alt="" aria-hidden="true" style={{ width: 24, height: 24, objectFit: 'contain', opacity: isActive ? 1 : 0.92 }} /><span>{label}</span>{badge && badge > 0 ? <span className="nav-badge">{badge > 9 ? '9+' : badge}</span> : null}{isActive && <div className="nav-active-line" />}</button>
        })}
      </nav>
    </div>
  )
