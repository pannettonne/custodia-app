...
export function NotesPanel() {
  ...
  // mark unread notes as read on mount
  useMemo(() => {
    notes.filter(n => !n.read && n.createdBy !== user?.uid).forEach(n => markNoteRead(n.id))
  }, [notes.length])
    ...
    <div>
      ...
      {notes.length === 0 && !showForm ? (
        <div className="empty-state">
          ...
          <div className="empty-state-title">Sin notas todavía</div>
          <div className="empty-state-sub">Añade notas sobre días concretos para el otro progenitor</div>
        </div>
      ) : (
  ...
  return (
    <div className="card" style={{ borderLeft: `3px solid ${tag.color}`, borderRadius: '0 16px 16px 0', marginBottom: 10 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
        ...
        {isOwn && (
          <button onClick={() => deleteNote(note.id)} style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', fontSize: 16, lineHeight: 1, padding: '0 2px' }}>✕</button>
        )}
      </div>
...
function NoteForm({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createNote({
        childId: child.id, createdBy: user.uid,
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
            ...
            <span style={{ fontSize: 12, color: '#9ca3af' }}>
              Notificar a <strong style={{ color: '#a78bfa' }}>{child.parentNames?.[child.parents.find(p => p !== user?.uid) ?? ''] ?? 'el otro progenitor'}</strong>
            </span>
          </label>
        </div>
      )}
