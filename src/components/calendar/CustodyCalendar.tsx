    ...
    <div>
      ...
      <div className="cal-grid">
        {days.map(date => {
          ...
          return <div key={date.toISOString()} className={`cal-cell${!inMonth?' other-month':''}${todayDay?' today':''}`} style={info ? { background: info.color+'28', borderColor: isSelected ? '#ffffffaa' : hasPendingRequest ? '#60a5fa' : (sp ? info.color+'99' : info.color+'55'), borderWidth: isSelected || sp || hasPendingRequest ? 2 : 1, borderStyle: hasPendingRequest ? 'dashed' : 'solid' } : { background:'rgba(255,255,255,0.03)', borderColor:isSelected ? '#ffffff88' : hasPendingRequest ? '#60a5fa' : 'rgba(255,255,255,0.07)', borderWidth:isSelected || hasPendingRequest ? 2 : 1, borderStyle: hasPendingRequest ? 'dashed' : 'solid' }} onClick={() => { if(inMonth){ setSelectedDate(dateStr) } }}><div className="cal-day-num">{format(date,'d')}</div>{info && <div className="cal-day-name" style={{color:info.color}}>{info.name.split(' ')[0].slice(0,4)}</div>}{isOverride && <div className="cal-modified-dot" />}{hasPendingRequest && <div style={{position:'absolute',top:2,left:2,fontSize:8,color:'#60a5fa',fontWeight:800}}>P</div>}{sp && isSpecialStart && <div style={{position:'absolute',top:1,left: hasPendingRequest ? 10 : 1,width:5,height:5,borderRadius:'50%',background:'#fff',opacity:0.7}} />}{todayDay && <div className="cal-today-dot" />}{(hasNotes || hasEvents) && <div style={{ position:'absolute', bottom:4, left:'50%', transform:'translateX(-50%)', display:'flex', gap:4, alignItems:'center' }}>{hasNotes && <div style={{ width:6, height:6, borderRadius:'50%', background:'#f59e0b' }} />}{hasEvents && <div style={{ width:6, height:6, borderRadius:'50%', background:'#10b981' }} />}</div>}</div>
        })}
      </div>
      <div style={{marginTop:14,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}><div style={{display:'flex',alignItems:'center',gap:12,fontSize:11,color:'#6b7280', flexWrap:'wrap'}}><div style={{display:'flex',alignItems:'center',gap:4}}><div style={{width:8,height:8,borderRadius:'50%',background:'#fbbf24'}} />cambio puntual</div><div style={{display:'flex',alignItems:'center',gap:4}}><div style={{width:8,height:8,borderRadius:'50%',background:'#60a5fa'}} />solicitud pendiente</div><div style={{display:'flex',alignItems:'center',gap:4}}><div style={{width:8,height:8,borderRadius:'50%',background:'#fff',opacity:0.6}} />periodo especial</div><div style={{display:'flex',alignItems:'center',gap:4}}><div style={{width:8,height:8,borderRadius:'50%',background:'#f59e0b'}} />notas</div><div style={{display:'flex',alignItems:'center',gap:4}}><div style={{width:8,height:8,borderRadius:'50%',background:'#10b981'}} />eventos</div></div><div style={{display:'flex',gap:8}}><button onClick={handlePrint} style={{fontSize:12,color:'#10b981',background:'none',border:'none',cursor:'pointer',fontWeight:700}}>🖨️ Imprimir / PDF</button><button onClick={() => { setSelectedDate(null); setModalOpen(true) }} style={{fontSize:12,color:'#3B82F6',background:'none',border:'none',cursor:'pointer',fontWeight:600}}>+ Solicitar cambio</button></div></div>
      {selectedDate && <div className="card" style={{ marginTop: 14 }}><div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, marginBottom:12 }}><div><div style={{ fontSize: 14, fontWeight: 800, color:'#fff' }}>{formatDate(selectedDate)}</div>{selectedParentInfo && <div style={{ fontSize:12, color:'#9ca3af', marginTop:4 }}>Corresponde a <strong style={{ color:selectedParentInfo.color }}>{selectedParentInfo.name}</strong>{selectedParentInfo.isMe ? ' (tú)' : ''}</div>}</div><button onClick={() => setModalOpen(true)} style={{ padding:'9px 12px', borderRadius:10, border:'none', background:'#3B82F6', color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer' }}>Solicitar cambio</button></div>
        ...
        <div style={{ display:'grid', gap:10 }}>
          ...
          <div><div style={{ fontSize:12, fontWeight:700, color:'#10b981', marginBottom:6 }}>Eventos del día</div>{selectedEvents.length === 0 ? <div style={{ fontSize:12, color:'#6b7280' }}>No hay eventos para este día.</div> : <div style={{ display:'flex', flexDirection:'column', gap:8 }}>{selectedEvents.map(({ event, cancelled }) => <div key={event.id} style={{ padding:'10px 12px', borderRadius:12, background: cancelled ? 'rgba(239,68,68,0.10)' : 'rgba(16,185,129,0.10)', border: cancelled ? '1px solid rgba(239,68,68,0.25)' : '1px solid rgba(16,185,129,0.2)', opacity: cancelled ? 0.9 : 1 }}><div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}><div><div style={{ fontSize:12, color:'#e5e7eb', fontWeight:700 }}>{event.title}</div><div style={{ fontSize:11, color:'#9ca3af', marginTop:4 }}>{event.allDay ? 'Todo el día' : (event.time || 'Sin hora')} · {event.customCategory || event.category}</div></div>{event.recurrence && event.recurrence !== 'none' && event.createdBy === user?.uid && <button onClick={() => handleToggleOccurrence(event.id, cancelled)} disabled={eventActionLoading === event.id} style={{ background:'none', border:'none', color: cancelled ? '#10b981' : '#f87171', fontSize:12, fontWeight:700, cursor:'pointer' }}>{eventActionLoading === event.id ? '...' : cancelled ? 'Restaurar' : 'Cancelar este día'}</button>}</div>{cancelled && <div style={{ fontSize:11, color:'#f87171', marginTop:6, fontWeight:700 }}>Cancelado</div>}{event.notes && <div style={{ fontSize:11, color:'#cbd5e1', marginTop:6 }}>{event.notes}</div>}</div>)}</div>}</div>
          <div><div style={{ fontSize:12, fontWeight:700, color:'#60a5fa', marginBottom:6 }}>Solicitudes de cambio</div>{selectedRequests.length === 0 ? <div style={{ fontSize:12, color:'#6b7280' }}>No hay solicitudes para este día.</div> : <div style={{ display:'flex', flexDirection:'column', gap:8 }}>{selectedRequests.map(req => <div key={req.id} style={{ padding:'10px 12px', borderRadius:12, background:'rgba(59,130,246,0.10)', border:'1px solid rgba(59,130,246,0.2)' }}><div style={{ fontSize:12, color:'#e5e7eb', fontWeight:700 }}>{req.fromParentName}</div><div style={{ fontSize:11, color:'#9ca3af', marginTop:4 }}>{req.reason}</div><div style={{ fontSize:11, color:req.status === 'pending' ? '#fbbf24' : req.status === 'accepted' ? '#10b981' : req.status === 'cancelled' ? '#9ca3af' : '#f87171', marginTop:6, fontWeight:700 }}>{req.status.toUpperCase()}</div></div>)}</div>}</div>
        </div>
      </div>}
