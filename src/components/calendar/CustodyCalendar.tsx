...
export function CustodyCalendar() {
  ...
  const selectedRequests = useMemo(() => selectedDate ? requests.filter(r =>
    (r.type === 'single' && r.date === selectedDate) ||
    (r.type === 'range' && r.startDate && r.endDate && selectedDate >= r.startDate && selectedDate <= r.endDate)
  ) : [], [requests, selectedDate])
  const selectedParentInfo = useMemo(() => {
    if (!selectedDate || !child || !pattern) return null
    return getParentInfo(new Date(selectedDate + 'T12:00:00'))
  }, [selectedDate, child, pattern, getParentInfo])
    ...
    <div>
        ...
        {days.map(date => {
          ...
          return (
            <div key={date.toISOString()}
              ...
              style={info
                ? { background: info.color+'28', borderColor: isSelected ? '#ffffffaa' : (sp ? info.color+'99' : info.color+'55'), borderWidth: isSelected || sp ? 2 : 1 }
                : { background:'rgba(255,255,255,0.03)', borderColor:isSelected ? '#ffffff88' : 'rgba(255,255,255,0.07)', borderWidth:isSelected ? 2 : 1 }
              }
              onClick={() => { if(inMonth){ setSelectedDate(dateStr) } }}
            >
              ...
              {(hasNotes || hasEvents) && (
                <div style={{ position:'absolute', bottom:4, left:'50%', transform:'translateX(-50%)', display:'flex', gap:4, alignItems:'center' }}>
                  {hasNotes && <div style={{ width:6, height:6, borderRadius:'50%', background:'#f59e0b' }} />}
                  {hasEvents && <div style={{ width:6, height:6, borderRadius:'50%', background:'#10b981' }} />}
                </div>
              )}
            </div>
          )
      ...
      <div style={{marginTop:14,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:12,fontSize:11,color:'#6b7280', flexWrap:'wrap'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:4}}>
            <div style={{width:8,height:8,borderRadius:'50%',background:'#f59e0b'}} />notas
          </div>
          <div style={{display:'flex',alignItems:'center',gap:4}}>
            <div style={{width:8,height:8,borderRadius:'50%',background:'#10b981'}} />eventos
          </div>
        </div>
      ...
      {selectedDate && (
        <div className="card" style={{ marginTop: 14 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, marginBottom:12 }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color:'#fff' }}>{formatDate(selectedDate)}</div>
              {selectedParentInfo && (
                <div style={{ fontSize:12, color:'#9ca3af', marginTop:4 }}>
                  Corresponde a <strong style={{ color:selectedParentInfo.color }}>{selectedParentInfo.name}</strong>{selectedParentInfo.isMe ? ' (tú)' : ''}
                </div>
              )}
            </div>
            <button onClick={() => setModalOpen(true)} style={{ padding:'9px 12px', borderRadius:10, border:'none', background:'#3B82F6', color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer' }}>
              Solicitar cambio
            </button>
          </div>

          {selectedOverride && (
            <div style={{ marginBottom:10, padding:'10px 12px', borderRadius:12, background:'rgba(251,191,36,0.12)', border:'1px solid rgba(251,191,36,0.25)' }}>
              <div style={{ fontSize:12, fontWeight:700, color:'#fbbf24', marginBottom:4 }}>Cambio puntual aplicado</div>
              <div style={{ fontSize:12, color:'#d1d5db' }}>{selectedOverride.reason || 'Sin observaciones'}</div>
            </div>
          )}

          {selectedSpecialPeriod && (
            <div style={{ marginBottom:10, padding:'10px 12px', borderRadius:12, background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ fontSize:12, fontWeight:700, color:'#fff', marginBottom:4 }}>
                {selectedSpecialPeriod.label === 'otro' ? (selectedSpecialPeriod.customLabel ?? 'Período especial') : PERIOD_LABELS[selectedSpecialPeriod.label]}
              </div>
              <div style={{ fontSize:12, color:'#9ca3af' }}>Periodo especial activo este día</div>
            </div>
          )}
          ...
          <div style={{ display:'grid', gap:10 }}>
            <div>
              ...
              {selectedNotes.length === 0 ? (
                <div style={{ fontSize:12, color:'#6b7280' }}>No hay notas para este día.</div>
              ) : (
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {selectedNotes.map(note => (
                    <div key={note.id} style={{ padding:'10px 12px', borderRadius:12, background:'rgba(245,158,11,0.10)', border:'1px solid rgba(245,158,11,0.2)' }}>
                      <div style={{ fontSize:11, color:'#f59e0b', fontWeight:700, marginBottom:4 }}>{note.tag.toUpperCase()}</div>
                      <div style={{ fontSize:12, color:'#e5e7eb' }}>{note.text}</div>
                      <div style={{ fontSize:11, color:'#9ca3af', marginTop:6 }}>Por {note.createdByName}{note.mentionOther ? ' · notifica al otro progenitor' : ''}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              ...
              {selectedEvents.length === 0 ? (
                <div style={{ fontSize:12, color:'#6b7280' }}>No hay eventos para este día.</div>
              ) : (
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {selectedEvents.map(event => (
                    <div key={event.id} style={{ padding:'10px 12px', borderRadius:12, background:'rgba(16,185,129,0.10)', border:'1px solid rgba(16,185,129,0.2)' }}>
                      <div style={{ fontSize:12, color:'#e5e7eb', fontWeight:700 }}>{event.title}</div>
                      <div style={{ fontSize:11, color:'#9ca3af', marginTop:4 }}>
                        {event.allDay ? 'Todo el día' : (event.time || 'Sin hora')} · {event.category}
                      </div>
                      {event.notes && <div style={{ fontSize:11, color:'#cbd5e1', marginTop:6 }}>{event.notes}</div>}
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              ...
              {selectedRequests.length === 0 ? (
                <div style={{ fontSize:12, color:'#6b7280' }}>No hay solicitudes para este día.</div>
              ) : (
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {selectedRequests.map(req => (
                    <div key={req.id} style={{ padding:'10px 12px', borderRadius:12, background:'rgba(59,130,246,0.10)', border:'1px solid rgba(59,130,246,0.2)' }}>
                      <div style={{ fontSize:12, color:'#e5e7eb', fontWeight:700 }}>{req.fromParentName}</div>
                      <div style={{ fontSize:11, color:'#9ca3af', marginTop:4 }}>{req.reason}</div>
                      <div style={{ fontSize:11, color:req.status === 'pending' ? '#fbbf24' : req.status === 'accepted' ? '#10b981' : '#f87171', marginTop:6, fontWeight:700 }}>
