    ...
    return (
      <div style={{ marginTop: 16, display:'grid', gap:12 }}>
        ...
        <div style={{ display:'grid', gap:12 }}>
          <DetailSection title="Notas" color="#f59e0b" empty="No hay notas para este día.">
            {selectedNotes.length > 0 && <div style={{ display:'grid', gap:8 }}>{selectedNotes.map(note => <div key={note.id} style={{ padding:'10px 12px', borderRadius:14, background:'rgba(245,158,11,0.10)', border:'1px solid rgba(245,158,11,0.2)' }}><div style={{ fontSize:11, color:'#f59e0b', fontWeight:800, marginBottom:4 }}>{note.tag.toUpperCase()}</div><div style={{ fontSize:13, color:'var(--text-strong)' }}>{note.text}</div><div style={{ fontSize:11, color:'var(--text-secondary)', marginTop:6 }}>Por {note.createdByName}{note.mentionOther ? ' · notifica al otro progenitor' : ''}</div></div>)}</div>}
          </DetailSection>

          <DetailSection title="Eventos" color="#10b981" empty="No hay eventos para este día.">
            {selectedEvents.length > 0 && <div style={{ display:'grid', gap:8 }}>{selectedEvents.map(({ event, cancelled }) => <div key={event.id} style={{ padding:'10px 12px', borderRadius:14, background: cancelled ? 'rgba(239,68,68,0.10)' : 'rgba(16,185,129,0.10)', border: cancelled ? '1px solid rgba(239,68,68,0.25)' : '1px solid rgba(16,185,129,0.2)', opacity: cancelled ? 0.9 : 1 }}><div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}><div><div style={{ fontSize:13, color:'var(--text-strong)', fontWeight:800 }}>{event.title}</div><div style={{ fontSize:11, color:'var(--text-secondary)', marginTop:4 }}>{event.allDay ? 'Todo el día' : (event.time || 'Sin hora')} · {event.customCategory || event.category}</div></div>{event.recurrence && event.recurrence !== 'none' && event.createdBy === user?.uid && <button onClick={() => handleToggleOccurrence(event.id, cancelled)} disabled={eventActionLoading === event.id} style={{ background:'none', border:'none', color: cancelled ? '#10b981' : '#f87171', fontSize:12, fontWeight:800, cursor:'pointer' }}>{eventActionLoading === event.id ? '...' : cancelled ? 'Restaurar' : 'Cancelar'}</button>}</div>{cancelled && <div style={{ fontSize:11, color:'#f87171', marginTop:6, fontWeight:800 }}>Cancelado</div>}{event.notes && <div style={{ fontSize:11, color:'var(--text-secondary)', marginTop:6 }}>{event.notes}</div>}</div>)}</div>}
          </DetailSection>

          <DetailSection title="Solicitudes de cambio" color="#60a5fa" empty="No hay solicitudes para este día.">
            {selectedRequests.length > 0 && <div style={{ display:'grid', gap:8 }}>{selectedRequests.map(req => <div key={req.id} style={{ padding:'10px 12px', borderRadius:14, background:'rgba(59,130,246,0.10)', border:'1px solid rgba(59,130,246,0.2)' }}><div style={{ fontSize:13, color:'var(--text-strong)', fontWeight:800 }}>{req.fromParentName}</div><div style={{ fontSize:11, color:'var(--text-secondary)', marginTop:4 }}>{req.reason}</div><div style={{ fontSize:11, color:req.status === 'pending' ? '#fbbf24' : req.status === 'accepted' ? '#10b981' : req.status === 'cancelled' ? 'var(--text-muted)' : '#f87171', marginTop:6, fontWeight:800 }}>{req.status.toUpperCase()}</div></div>)}</div>}
          </DetailSection>
        </div>
      </div>
    )
    ...
    <div>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
          ...
          <div style={{ textAlign:'center', flex:1 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, flexWrap:'wrap', marginTop:8 }}>
              {child.parents.map(pid => <div key={pid} style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 9px', borderRadius:999, background:'var(--bg-card)', border:'1px solid var(--border)' }}><div style={{ width:8, height:8, borderRadius:'50%', background:child.parentColors?.[pid] ?? '#6B7280' }} /><span style={{ fontSize:11, color:'var(--text-secondary)', fontWeight:700 }}>{child.parentNames?.[pid] ?? 'Progenitor'}{pid===user?.uid ? ' · tú' : ''}</span></div>)}
            </div>
          </div>
      ...
      {viewMode === 'month' && (
        <>
          ...
          <div className="cal-grid" style={{ gap:8 }}>
            {days.map(date => {
        ...
        <>
          <div style={{ display:'grid', gap:10 }}>
            {weekDays.map(day => {
              ...
              const isSelected = selectedDate === dateStr
              return <button key={dateStr} onClick={() => syncToDate(dateStr)} style={{ textAlign:'left', padding:'14px', borderRadius:20, border:`1px solid ${isSelected ? '#3B82F6' : 'var(--border)'}`, background:isSelected ? 'linear-gradient(180deg, rgba(59,130,246,0.14) 0%, var(--bg-card) 100%)' : (info ? info.color+'12' : 'var(--bg-card)'), color:'inherit', cursor:'pointer', boxShadow:isSelected ? '0 10px 24px rgba(59,130,246,0.14)' : 'none' }}><div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:10, marginBottom:8 }}><div><div style={{ fontSize:11, color:'var(--text-muted)', textTransform:'uppercase', fontWeight:800 }}>{format(day, 'EEEE', { locale: es })}</div><div style={{ fontSize:18, fontWeight:900, color:'var(--text-strong)' }}>{format(day, "d 'de' MMM", { locale: es })}</div></div>{info && <div style={{ fontSize:12, fontWeight:800, color:info.color }}>{info.name}</div>}</div><div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>{itemsEvents.length > 0 && <span style={{ fontSize:11, color:'#10b981', fontWeight:800, padding:'4px 8px', borderRadius:999, background:'rgba(16,185,129,0.10)' }}>{itemsEvents.length} evento(s)</span>}{itemsNotes.length > 0 && <span style={{ fontSize:11, color:'#f59e0b', fontWeight:800, padding:'4px 8px', borderRadius:999, background:'rgba(245,158,11,0.10)' }}>{itemsNotes.length} nota(s)</span>}{pendingCount > 0 && <span style={{ fontSize:11, color:'#60a5fa', fontWeight:800, padding:'4px 8px', borderRadius:999, background:'rgba(96,165,250,0.10)' }}>{pendingCount} cambio(s)</span>}{itemsEvents.length === 0 && itemsNotes.length === 0 && pendingCount === 0 && <span style={{ fontSize:11, color:'var(--text-muted)' }}>Sin actividad</span>}</div></button>
            })}
          </div>
          {renderDayDetail()}
        </>
