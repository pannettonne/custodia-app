    ...
    <div>
      ...
      {selectedDate && <div className="card" style={{ marginTop: 14 }}><div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, marginBottom:12 }}><div><div style={{ fontSize: 14, fontWeight: 800, color:'var(--text-strong)' }}>{formatDate(selectedDate)}</div>{selectedParentInfo && <div style={{ fontSize:12, color:'var(--text-secondary)', marginTop:4 }}>Corresponde a <strong style={{ color:selectedParentInfo.color }}>{selectedParentInfo.name}</strong>{selectedParentInfo.isMe ? ' (tú)' : ''}</div>}</div><button onClick={() => setModalOpen(true)} style={{ padding:'9px 12px', borderRadius:10, border:'none', background:'#3B82F6', color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer' }}>Solicitar cambio</button></div>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom: 12 }}>
          <button onClick={() => setModalOpen(true)} style={{ padding:'8px 10px', borderRadius:10, border:'1px solid var(--border)', background:'var(--bg-soft)', color:'var(--text-secondary)', fontSize:12, fontWeight:700, cursor:'pointer' }}>🔄 Solicitar cambio</button>
          <button onClick={() => alert('Ve a Notas para añadir una nota en este día.')} style={{ padding:'8px 10px', borderRadius:10, border:'1px solid var(--border)', background:'var(--bg-soft)', color:'var(--text-secondary)', fontSize:12, fontWeight:700, cursor:'pointer' }}>📝 Añadir nota</button>
          <button onClick={() => alert('Ve a Eventos para añadir un evento en este día.')} style={{ padding:'8px 10px', borderRadius:10, border:'1px solid var(--border)', background:'var(--bg-soft)', color:'var(--text-secondary)', fontSize:12, fontWeight:700, cursor:'pointer' }}>🎓 Añadir evento</button>
        </div>
