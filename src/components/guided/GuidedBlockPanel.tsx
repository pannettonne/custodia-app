...
import { createAvailabilityBlock, updateAvailabilityBlock } from '@/lib/availability-blocks-db'
import { showToast } from '@/lib/toast'
import type { AvailabilityBlock, AvailabilityBlockType } from '@/types'

type Props = { onBack: () => void; onDone?: () => void; editItem?: AvailabilityBlock | null }
...
export function GuidedBlockPanel({ onBack, onDone, editItem = null }: Props) {
  const { user } = useAuth()
  ...
  const [type, setType] = useState<AvailabilityBlockType>(editItem?.type || 'full_day')
  ...
  const [endTime, setEndTime] = useState(editItem?.endTime || '14:00')
  ...
  const save = async () => {
    ...
    try {
      const payload: Partial<AvailabilityBlock> = {
        childId: child.id,
        userId: editItem?.userId || user.uid,
        userName: editItem?.userName || user.displayName || user.email || 'Usuario',
        ownerRole: editItem?.ownerRole || (isParent ? 'parent' : 'collaborator'),
        type,
      ...
      if (isEditing && editItem) {
        await updateAvailabilityBlock(editItem.id, payload)
        showToast({ message: 'Bloqueo actualizado.', tone: 'success' })
      ...
      await createAvailabilityBlock(payload as Omit<AvailabilityBlock, 'id' | 'createdAt'>)
      showToast({ message: 'Bloqueo guardado.', tone: 'success' })
      window.dispatchEvent(new CustomEvent('custodia:navigate', { detail: { tab: 'requests', childId: child.id, date: type === 'date_range' ? startDate : date } }))
    } catch (err) {
      const message = err instanceof Error ? err.message : isEditing ? 'No se pudo actualizar el bloqueo.' : 'No se pudo guardar el bloqueo.'
      setError(message)
  ...
  const body = () => {
    ...
    const rows = [['Menor', child?.name || '—'], ['Bloquea a', editItem?.userName || user?.displayName || user?.email || 'Usuario'], ['Tipo', type === 'full_day' ? 'Día completo' : type === 'date_range' ? 'Rango de fechas' : 'Horario concreto'], ['Cuándo', summaryDate], ['Motivo', note.trim() || 'Sin motivo']]
