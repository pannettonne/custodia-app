...
export function GuidedCreationPanel() {
  ...
  const body = () => {
    if (step === 0) return <div style={{ display: 'grid', gap: 10 }}>{['Evento', 'Cambio', 'Bloqueo', 'Tratamiento', 'Nota'].map((name, i) => <button key={name} type="button" onClick={() => i === 0 ? next() : showToast({ message: 'Ese flujo lo añadiremos después.', tone: 'info' })} style={{ padding: 12, borderRadius: 18, border: `1px solid ${i === 0 ? '#7c3aed' : 'var(--border)'}`, background: i === 0 ? 'rgba(124,58,237,.10)' : 'var(--bg-card)', color: 'var(--text-strong)', textAlign: 'left' }}><strong>{name}</strong><br /><small style={{ color: 'var(--text-secondary)' }}>{i === 0 ? 'Citas, colegio, actividades y vacaciones.' : 'Próximamente.'}</small></button>)}</div>
