...
import { updateChangeRequest } from '@/lib/change-requests-db'
...
type Props = { onBack: () => void; onDone?: () => void; editItem?: ChangeRequest | null }
...
export function GuidedChangePanel({ onBack, onDone, editItem = null }: Props) {
  const { user } = useAuth()
  const { children, selectedChildId } = useAppStore()
  const child = useMemo(() => children.find(c => c.id === selectedChildId) || null, [children, selectedChildId])
  const otherParentId = useMemo(() => editItem?.toParentId || child?.parents.find(p => p !== user?.uid) || null, [child, user, editItem])
  const otherParentName = otherParentId ? child?.parentNames?.[otherParentId] || 'El otro progenitor' : '—'
  const isEditing = !!editItem?.id
  const [step, setStep] = useState(0)
  const [type, setType] = useState<'single' | 'range'>(editItem?.type || 'single')
  ...
  const [locationAddress, setLocationAddress] = useState(editItem?.locationAddress || '')
  const [locationLatitude, setLocationLatitude] = useState<number | undefined>(editItem?.locationLatitude)
  const [locationLongitude, setLocationLongitude] = useState<number | undefined>(editItem?.locationLongitude)
  const [locationPlaceId, setLocationPlaceId] = useState(editItem?.locationPlaceId || '')
  ...
  const send = async () => {
    ...
    try {
      ...
      const payload: any = { childId: child.id, fromParentId: editItem?.fromParentId || user.uid, fromParentName: editItem?.fromParentName || user.displayName || user.email || 'Progenitor', toParentId: otherParentId, type, ...(type === 'single' ? { date } : { startDate, endDate }), reason: reason.trim(), locationName: locationName.trim() || undefined, locationAddress: locationAddress.trim() || undefined, locationLatitude, locationLongitude, locationPlaceId: locationPlaceId || undefined }
      if (isEditing && editItem) {
        await updateChangeRequest(editItem.id, payload)
        showToast({ message: 'Solicitud actualizada.', tone: 'success' })
      ...
      const message = e instanceof Error ? e.message : isEditing ? 'No se pudo actualizar la solicitud.' : 'No se pudo enviar la solicitud.'
      setError(message)
