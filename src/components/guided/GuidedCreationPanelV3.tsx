...
const questionFor = (step: number) => ['¿Qué quieres crear?', '¿Para quién es?', '¿Qué tipo de evento es?', '¿Cómo lo llamamos?', '¿Cuándo será?', '¿Dónde será?', '¿Quieres añadir algo más?', '¿Está todo correcto?'][step] || ''
...
export function GuidedCreationPanel() {
  ...
  const renderChoices = () => {
    ...
    return <div style={{ display: 'grid', gap: 10 }}>{items.map(([icon, name, subtitle, enabled]) => <button key={name} type="button" onClick={() => enabled ? setStep(1) : showToast({ message: 'Ese flujo lo añadiremos después.', tone: 'info' })} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, borderRadius: 18, border: `1px solid ${enabled ? '#7c3aed' : 'var(--border)'}`, background: enabled ? 'rgba(124,58,237,.10)' : 'var(--bg-card)', color: 'var(--text-strong)', cursor: 'pointer', textAlign: 'left' }}><span style={{ fontSize: 22 }}>{icon}</span><span style={{ flex: 1 }}><strong>{name}</strong><br /><small style={{ color: 'var(--text-secondary)' }}>{subtitle}</small></span><span style={{ color: enabled ? '#7c3aed' : 'var(--text-muted)', fontSize: enabled ? 16 : 10, fontWeight: 900 }}>{enabled ? '›' : 'Pronto'}</span></button>)}</div>
  }
  ...
  const renderBody = () => {
    ...
    if (step === 2) return <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 8 }}>{Object.entries(CAT_CONFIG).map(([key, value]) => <button key={key} type="button" onClick={() => { const next = key as EventCategory; setCategory(next); setTitle(value.label === 'Médico' ? 'Cita médica' : value.label) }} style={{ minHeight: 86, borderRadius: 18, border: `1px solid ${category === key ? value.color : 'var(--border)'}`, background: category === key ? `${value.color}20` : 'var(--bg-card)', color: category === key ? value.color : 'var(--text-secondary)', cursor: 'pointer' }}><div style={{ fontSize: 24 }}>{value.icon}</div><strong>{value.label}</strong></button>)}</div>
    ...
    const rows = [['Tipo', CAT_CONFIG[category]?.label || 'Evento'], ['Para', child?.name || '—'], ['Título', title], ['Fecha', endDate ? `${date} → ${endDate}` : date], ['Hora', allDay ? 'Todo el día' : `${time}${endTime ? `-${endTime}` : ''}`], ['Lugar', locationName || locationAddress || 'Sin lugar'], ['Recordatorio', reminderEnabled ? 'Activado' : 'No']]
