...
import { createMedicationPlan } from '@/lib/medications-db'
...
export function GuidedTreatmentPanel({ onBack }: Props) {
  const { user } = useAuth()
  ...
  const save = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'No se pudo guardar el tratamiento.'
      setError(message)
