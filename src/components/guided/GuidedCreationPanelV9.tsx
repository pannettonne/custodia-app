...
type GuidedEditTarget = { type: 'event' | 'change' | 'block' | 'treatment' | 'note'; item: any }
type Props = { editTarget?: GuidedEditTarget | null; onDone?: () => void }
...
export function GuidedCreationPanel({ editTarget = null, onDone }: Props) {
  const { user } = useAuth()
  const { children, selectedChildId, setSelectedChildId } = useAppStore()
  const child = useMemo(() => children.find(c => c.id === selectedChildId) || null, [children, selectedChildId])
  const editingEvent = editTarget?.type === 'event' ? editTarget.item : null
  const isEditing = !!editingEvent

  const [mode, setMode] = useState<'menu' | 'event' | 'change' | 'block' | 'treatment' | 'note'>(editTarget?.type || 'menu')
  const [step, setStep] = useState(isEditing ? 1 : 0)
  const [category, setCategory] = useState<EventCategory>((editingEvent?.category as EventCategory) || 'medico')
  const [title, setTitle] = useState(editingEvent?.title || 'Cita médica')
  const [date, setDate] = useState(editingEvent?.date || today())
  const [endDate, setEndDate] = useState(editingEvent?.endDate || '')
  const [allDay, setAllDay] = useState(editingEvent?.allDay ?? true)
  const [time, setTime] = useState(editingEvent?.time || '')
  const [endTime, setEndTime] = useState(editingEvent?.endTime || '')
  const [locationName, setLocationName] = useState(editingEvent?.locationName || '')
  const [locationAddress, setLocationAddress] = useState(editingEvent?.locationAddress || '')
  const [locationLatitude, setLocationLatitude] = useState<number | undefined>(editingEvent?.locationLatitude)
  const [locationLongitude, setLocationLongitude] = useState<number | undefined>(editingEvent?.locationLongitude)
  const [locationPlaceId, setLocationPlaceId] = useState(editingEvent?.locationPlaceId || '')
  const [assignedParentId, setAssignedParentId] = useState(editingEvent?.assignedParentId || '')
  const [reminderEnabled, setReminderEnabled] = useState(editingEvent?.reminderEnabled ?? false)
  const [reminderDaysBefore, setReminderDaysBefore] = useState(editingEvent?.reminderDaysBefore ?? 1)
  const [reminderAudience, setReminderAudience] = useState<EventReminderAudience>(editingEvent?.reminderAudience || 'self')
  const [notesEnabled, setNotesEnabled] = useState(!!editingEvent?.notes)
  const [notes, setNotes] = useState(editingEvent?.notes || '')
  const [documentsEnabled, setDocumentsEnabled] = useState((editingEvent?.documentIds || []).length > 0)
  const [documentIds, setDocumentIds] = useState<string[]>(editingEvent?.documentIds || [])
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')

  if (mode === 'change') return <GuidedChangePanel editItem={editTarget?.type === 'change' ? editTarget.item : null} onBack={() => editTarget ? onDone?.() : setMode('menu')} onDone={onDone} />
  if (mode === 'block') return <GuidedBlockPanel editItem={editTarget?.type === 'block' ? editTarget.item : null} onBack={() => editTarget ? onDone?.() : setMode('menu')} onDone={onDone} />
  if (mode === 'treatment') return <GuidedTreatmentPanel editItem={editTarget?.type === 'treatment' ? editTarget.item : null} onBack={() => editTarget ? onDone?.() : setMode('menu')} onDone={onDone} />
  if (mode === 'note') return <GuidedNotePanel editItem={editTarget?.type === 'note' ? editTarget.item : null} onBack={() => editTarget ? onDone?.() : setMode('menu')} onDone={onDone} />
  ...
  async function save() {
    ...
    try {
      ...
      const payload: any = { childId: child.id, createdBy: editingEvent?.createdBy || user.uid, title: title.trim(), category, date, endDate: endDate || undefined, allDay, time: allDay ? undefined : time || undefined, endTime: allDay ? undefined : endTime || undefined, notes: notesEnabled ? notes.trim() || undefined : undefined, documentIds: documentsEnabled ? documentIds : [], recurrence: editingEvent?.recurrence || 'none', assignedParentId: wantsAssignment ? assignedParentId : editingEvent?.assignedParentId, assignmentStatus: wantsAssignment ? (editingEvent?.assignmentStatus || 'pending') : editingEvent?.assignmentStatus, assignmentRequestedBy: wantsAssignment ? (editingEvent?.assignmentRequestedBy || user.uid) : editingEvent?.assignmentRequestedBy, assignmentRequestedByName: wantsAssignment ? (editingEvent?.assignmentRequestedByName || user.displayName || user.email || 'Progenitor') : editingEvent?.assignmentRequestedByName, assignmentRequestToParentId: wantsAssignment ? (editingEvent?.assignmentRequestToParentId || otherParentId) : editingEvent?.assignmentRequestToParentId, reminderEnabled, reminderDaysBefore: reminderEnabled ? reminderDaysBefore : undefined, reminderAudience: reminderEnabled ? reminderAudience : undefined, locationName: locationName.trim() || undefined, locationAddress: locationAddress.trim() || undefined, locationLatitude, locationLongitude, locationPlaceId: locationPlaceId || undefined }
      if (editingEvent?.id) {
        await updateEvent(editingEvent.id, payload)
        showToast({ message: 'Evento actualizado.', tone: 'success' })
        window.dispatchEvent(new CustomEvent('custodia:navigate', { detail: { tab: 'events', childId: child.id, date, focusTargetId: `event-${editingEvent.id}` } }))
        onDone?.()
        return
      }
      ...
      const message = err instanceof Error ? err.message : (isEditing ? 'No se pudo actualizar el evento.' : 'No se pudo guardar el evento.')
  ...
  const body = () => {
    ...
    if (step === 1) return <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 8 }}>{Object.entries(CAT_CONFIG).map(([k, v]) => <button key={k} type="button" onClick={() => { setCategory(k as EventCategory); if (!isEditing) setTitle(v.label === 'Médico' ? 'Cita médica' : v.label) }} style={{ minHeight: 64, borderRadius: 16, border: `1px solid ${category === k ? v.color : 'var(--border)'}`, background: category === k ? `${v.color}1f` : 'var(--bg-card)', color: category === k ? v.color : 'var(--text-secondary)' }}><span style={{ fontSize: 16 }}>{v.icon}</span><br /><strong style={{ fontSize: 13 }}>{v.label}</strong></button>)}</div>
  ...
  return <section style={{ display: 'grid', gap: 14, width: '100%', maxWidth: 620, margin: '0 auto', paddingBottom: 18 }}><div className="card" style={{ padding: 18, borderRadius: 26, background: 'linear-gradient(180deg, var(--bg-card) 0%, var(--bg-soft) 100%)', border: '1px solid rgba(124,58,237,0.24)' }}><div style={{ color: '#7c3aed', fontSize: 10, fontWeight: 950, letterSpacing: .5, textTransform: 'uppercase', marginBottom: 5 }}>{isEditing ? 'Edición guiada' : 'Creación guiada'}</div><h1 style={{ margin: 0, color: 'var(--text-strong)', fontSize: 24, lineHeight: 1.05 }}>{isEditing ? 'Editar evento' : 'Una pregunta cada vez'}</h1><p style={{ margin: '8px 0 0', color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.45 }}>{isEditing ? `Modifica ${title || 'este evento'} paso a paso.` : mode === 'menu' ? 'Elige qué quieres crear.' : `Crea un evento para ${child?.name || 'el menor seleccionado'} paso a paso.`}</p></div><div className="card" style={{ padding: 16, borderRadius: 26, background: 'var(--bg-card)', border: '1px solid var(--border)' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}><span style={{ background: 'rgba(124,58,237,.12)', color: '#8b5cf6', borderRadius: 999, padding: '6px 9px', fontSize: 11, fontWeight: 950 }}>Paso {step + 1} de {STEPS.length}</span><span style={{ color: 'var(--text-muted)', fontSize: 11, fontWeight: 850 }}>{STEPS[step]}</span></div><div style={{ display: 'flex', gap: 5, marginBottom: 18 }}>{STEPS.map((x, i) => <span key={x} style={bar(i, step)} />)}</div><div style={{ marginBottom: 16 }}><div style={{ color: '#7c3aed', fontSize: 11, fontWeight: 950, textTransform: 'uppercase', marginBottom: 6 }}>{STEPS[step]}</div><h2 style={{ margin: 0, color: 'var(--text-strong)', fontSize: 21 }}>{QUESTIONS[step]}</h2></div>{body()}{error ? <div style={{ marginTop: 12, padding: 9, borderRadius: 12, background: 'rgba(239,68,68,.12)', color: '#fca5a5', fontSize: 12, fontWeight: 800 }}>{error}</div> : null}<div style={{ display: 'flex', gap: 8, marginTop: 16 }}><button type="button" className="btn-primary btn-outline" style={{ flex: 1 }} onClick={() => isEditing && step === 1 ? onDone?.() : mode === 'event' && step === 1 ? setStep(0) : setStep(v => Math.max(0, v - 1))} disabled={(step === 0 && !isEditing) || saving}>Anterior</button>{step < STEPS.length - 1 ? <button type="button" style={{ flex: 1, padding: 11, borderRadius: 13, border: 'none', background: canNext ? 'linear-gradient(135deg,#7c3aed,#4f46e5)' : 'rgba(255,255,255,.08)', color: canNext ? '#fff' : '#6b7280', fontWeight: 900 }} onClick={next} disabled={!canNext || saving}>Siguiente</button> : <button type="button" style={{ flex: 1, padding: 11, borderRadius: 13, border: 'none', background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', color: '#fff', fontWeight: 900 }} onClick={save} disabled={saving}>{saving ? 'Guardando...' : isEditing ? 'Guardar cambios' : 'Guardar evento'}</button>}</div></div></section>
}
