...
export function RequestsList() {
  ...
  const handleReject = async (req: ChangeRequest) => { await respondToRequest(req.id, 'rejected'); await notifyRequester(req, false) }
  const handleCancelOwn = async (req: ChangeRequest) => { await cancelRequest(req.id); await createNotification({ userId: req.toParentId, childId: req.childId, childName: child?.name, type: 'pending_request', title: 'Solicitud de cambio cancelada', body: `${user?.displayName || user?.email || 'El otro progenitor'} ha cancelado una solicitud de cambio pendiente.`, dateKey: `change-request-cancel:${req.id}` }) }
  ...
  if (requests.length === 0 && grouped.incomingEventAssignments.length === 0 && grouped.outgoingEventAssignments.length === 0) return <div className="empty-state"><div className="empty-state-icon">💬</div><div className="empty-state-title">No hay solicitudes de cambio</div><div className="empty-state-sub">Las peticiones aparecerán aquí</div></div>
  ...
  const Card = ({ req, isIncoming }: { req: ChangeRequest; isIncoming: boolean }) => {
    ...
    const palette = req.status === 'pending' ? { border: 'rgba(245,158,11,0.30)', badgeBg: 'rgba(245,158,11,0.14)', badgeColor: '#f59e0b' } : req.status === 'accepted' ? { border: 'rgba(16,185,129,0.28)', badgeBg: 'rgba(16,185,129,0.14)', badgeColor: '#10b981' } : req.status === 'cancelled' ? { border: 'var(--border)', badgeBg: 'var(--bg-soft)', badgeColor: 'var(--text-muted)' } : { border: 'rgba(239,68,68,0.26)', badgeBg: 'rgba(239,68,68,0.14)', badgeColor: '#ef4444' }
    const badgeText = req.status === 'pending' ? 'Pendiente' : req.status === 'accepted' ? 'Aceptada' : req.status === 'cancelled' ? 'Cancelada' : 'Rechazada'
    return <div style={{ background:'linear-gradient(180deg, var(--bg-card) 0%, var(--bg-soft) 100%)', border: `1px solid ${palette.border}`, borderRadius: 20, padding: 16, marginBottom: 10, boxShadow: 'var(--card-shadow)' }}><div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, marginBottom: 10 }}><span style={{ display:'inline-flex', alignItems:'center', padding:'5px 10px', borderRadius: 999, background: palette.badgeBg, color: palette.badgeColor, fontSize: 11, fontWeight: 800 }}>{badgeText}</span></div><div style={{ color:'var(--text-strong)', fontSize: 14, fontWeight: 800, marginBottom: 6 }}>{isIncoming ? req.fromParentName : 'Tú'} pide cambio · {req.type === 'single' ? 'Día concreto' : 'Rango de fechas'}</div><div style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize: 12, color:'var(--text-secondary)', background:'var(--bg-soft)', border:'1px solid var(--border)', padding:'6px 10px', borderRadius: 12, marginBottom: 10 }}><span>📅</span><span style={{ fontWeight: 700 }}>{dateText}</span></div><div style={{ fontSize: 12, color:'var(--text-secondary)', lineHeight: 1.45 }}><span style={{ color:'var(--text-muted)' }}>Motivo: </span>{req.reason}</div>{isIncoming && req.status === 'pending' && <div style={{ display:'flex', gap:8, marginTop: 12 }}><button className="req-action-btn btn-reject" onClick={() => handleReject(req)}>✕ Rechazar</button><button className="req-action-btn btn-accept" onClick={() => handleAccept(req)}>✓ Aceptar</button></div>}{!isIncoming && req.status === 'pending' && <div style={{ display:'flex', gap:8, marginTop: 12 }}><button className="req-action-btn btn-reject" onClick={() => handleCancelOwn(req)}>Cancelar solicitud</button></div>}{req.status === 'cancelled' && <div style={{ display:'flex', gap:8, marginTop: 12 }}><button className="req-action-btn btn-reject" onClick={() => deleteRequest(req.id)}>Eliminar</button></div>}</div>
  }
